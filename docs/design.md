...
The base Compose configuration keeps the gateway internal-only for E2E and retains loopback simulator port `127.0.0.1:8080` for the local `go run .` workflow; Mountebank admin port 2525 is never published. The development-only `docker-compose.dev.yml` override adds the assessment-only gateway mapping `127.0.0.1:8090:8090`. `make dev-up` and `make dev-down` use both Compose files; `make dev-up` requires a free local port 8090 and does not stop an existing listener. `make e2e` uses the base file only, so its service-DNS runner does not bind host port 8090. `make fuzz` is a deterministic synthetic-payment demo smoke workflow—not randomized fuzzing—that bounded-polls `/readyz`, checks authorized, declined, and unavailable responses for odd, non-zero even, and zero final digits respectively, then prints status summaries and a bounded safe gateway-log snapshot. It is assessment-only and must never be used with real payment data.
